        ...
        for ($i = 0; $i < $countProperties; ++$i) {
            ...
            switch ($type) {
                ...
                case 0x1E: // null-terminated string prepended by dword string length
                    ...
                    $value      = \PHPExcel\Shared\Str::ConvertEncoding($value, 'UTF-8', $codePage);
                    $value      = rtrim($value);
                    break;
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            if (($fmtRuns = $this->sst[$index]['fmtRuns']) && !$this->readDataOnly) {
                ...
                for ($i = 0; $i <= $sstCount; ++$i) {
                    if (isset($fmtRuns[$i])) {
                        $text    = \PHPExcel\Shared\Str::Substring($this->sst[$index]['value'], $charPos, $fmtRuns[$i]['charPos'] - $charPos);
                        $charPos = $fmtRuns[$i]['charPos'];
                    } else {
                        $text = \PHPExcel\Shared\Str::Substring($this->sst[$index]['value'], $charPos, \PHPExcel\Shared\Str::CountCharacters($this->sst[$index]['value']));
                    }

                    if (\PHPExcel\Shared\Str::CountCharacters($text) > 0) {
...
//        echo '<b>_readObj()</b><br />';
    ...
    private static function encodeUTF16($string, $compressed = '')
    {
        ...
        return \PHPExcel\Shared\Str::ConvertEncoding($string, 'UTF-8', 'UTF-16LE');
    }
    ...
    {
        return \PHPExcel\Shared\Str::ConvertEncoding($string, 'UTF-8', $this->codepage);
    }
