...
class AdvancedValueBinder extends DefaultValueBinder implements IValueBinder
{
    ...
    public function bindValue(\PHPExcel\Cell $cell, $value = null)
    {
        ...
        if (is_string($value)) {
            $value = \PHPExcel\Shared\Str::SanitizeUTF8($value);
        }
        ...
        if ($dataType === DataType::TYPE_STRING && !$value instanceof \PHPExcel\RichText) {
            ...
            // Check for currency
            $currencyCode       = \PHPExcel\Shared\Str::getCurrencyCode();
            $decimalSeparator   = \PHPExcel\Shared\Str::getDecimalSeparator();
            $thousandsSeparator = \PHPExcel\Shared\Str::getThousandsSeparator();
